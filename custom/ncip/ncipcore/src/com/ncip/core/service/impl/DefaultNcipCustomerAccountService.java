...
import com.ncip.core.service.NcipCustomerAccountService;


/**
 * Default implementation of {@link CustomerAccountService}
 */
public class DefaultNcipCustomerAccountService extends DefaultCustomerAccountService implements NcipCustomerAccountService
{
	@SuppressWarnings("unused")
	private static final Logger LOG = Logger.getLogger(DefaultNcipCustomerAccountService.class);
	...
	@Override
	public void updateProfile(final CustomerModel customerModel, CustomerData customerData, final String titleCode, final String name, final String login)
			throws DuplicateUidException
	{
...
//      {
          ...
          b2bCustomerModel = (B2BCustomerModel) customerModel;
          b2bCustomerModel.setName(customerData.getName()); //會員名稱
          b2bCustomerModel.setEmail(customerData.getEmail()); // Email
          b2bCustomerModel.setPoc_line(customerData.getPoc_line()); // 聯絡人Line
          b2bCustomerModel.setPoc_wechat(customerData.getPoc_wechat()); // 聯絡人Wechat
          b2bCustomerModel.setCell_number(customerData.getCell_number()); // 聯絡人手機
          //b2bCustomerModel.setUid(login);
          
          
          
          if(b2bCustomerModel.getDefaultB2BUnit() != null) {
            
            b2bCustomerModel.getDefaultB2BUnit().setPhone1(customerData.getTel_number()); // 公司電話
            b2bCustomerModel.getDefaultB2BUnit().setFax(customerData.getFax_number()); // 公司傳真
            internalSaveB2BUnit(b2bCustomerModel.getDefaultB2BUnit()); // 儲存B2BUnit
//             //b2bCustomerModel.getDefaultB2BUnit().setEDate(value);
//             //b2bCustomerModel.getDefaultB2BUnit().setConno(value);
          }
...
//      super.updateProfile(customerModel, titleCode, name, login);
          ...
          b2bCustomerModel.setUid(login);
          b2bCustomerModel.setName(name);

          if (StringUtils.isNotBlank(titleCode))
          {
            b2bCustomerModel.setTitle(getUserService().getTitleForCode(titleCode));
          }
          else
          {
            b2bCustomerModel.setTitle(null);
          }    
      
      
      internalSaveB2BCustomer(b2bCustomerModel);
      getEventService().publishEvent(initializeEvent(new UpdatedProfileEvent(), b2bCustomerModel));
     ...
     * Saves the customer translating model layer exceptions regarding duplicate identifiers
     */
    protected void internalSaveB2BCustomer(final B2BCustomerModel b2bCustomerModel) throws DuplicateUidException
    {
        try
        {
     ...
     * Saves the customer translating model layer exceptions regarding duplicate identifiers
     */
    protected void internalSaveB2BUnit(final B2BUnitModel b2bUnitModel) throws DuplicateUidException
    {
        try
        {
        ...
        catch (final ModelSavingException e)
        {
            if (e.getCause() instanceof InterceptorException
                    && ((InterceptorException) e.getCause()).getInterceptor().getClass().equals(UniqueAttributesInterceptor.class))
            {
                throw new DuplicateUidException(b2bUnitModel.getUid(), e);
            }
            else
            {
                throw e;
