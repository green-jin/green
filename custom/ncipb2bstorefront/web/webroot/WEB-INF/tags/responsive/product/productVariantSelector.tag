...
<%--TODO--%>
<!--
<span class="text-danger">部分庫存不足，交期30天</span>-->
<%--<span class="text-danger">下單後生產，交期60天</span>--%>
...
<%--                class="fa fa-shopping-cart px-1"></i>加到購物車--%>
<%--        </button>--%>
<%--    </div>--%>
<%--</div>--%>

<%--<h6 class="text-success mt-2">本產品加入A級特惠方案</h6>--%>
<%--<h6 class="text-success">本產品加入B級特惠方案</h6>--%>
<%--<h6 class="text-success">本產品加入C級特惠方案</h6>--%>
        ...
        <c:forEach begin="1" end="${levels}" varStatus="loop">
            ...
            <c:choose>
                <c:when test="${productMatrix[0].parentVariantCategory.hasImage}">
                    <h6 class="my-3">${fn:escapeXml(productMatrix[0].parentVariantCategory.name)}</h6>
                                ...
                                <ul class="variant-list">
                                    <c:forEach items="${productMatrix}" var="variantCategory">
                                        <li
                                                ...
                                                <%--                            <c:if test="${variantCategory.variantOption.url eq product.url}"></c:if>--%>
                                            <c:url value="${variantCategory.variantOption.url}"
                                                   var="productStyleUrl"/>
                                            <a href="${fn:escapeXml(productStyleUrl)}"
                                               class="swatchVariant"
                ...
                <%-- 辨識項目沒有Image: match --%>
                <c:otherwise>
                    <c:if test="${not empty fn:escapeXml(productMatrix[0].parentVariantCategory)}">
                    <select class="custom-select justify-content-start my-1"
                            id="priority${loop.index}">
                        <option selected>${fn:escapeXml(productMatrix[0].parentVariantCategory.name)}</option>
                        <c:forEach items="${productMatrix}" var="variantCategory">
                            <c:url value="${variantCategory.variantOption.url}"
                                   var="productStyleUrl"/>
...
<%--    TODO 以下尚未研究--%>
<%--    <c:otherwise>--%>
<%--        &lt;%&ndash; Determine if product is one of apparel style or size variant &ndash;%&gt;--%>
...
<%--        &lt;%&ndash; Determine if product is other variant &ndash;%&gt;--%>
...
<%--                                </c:forEach>--%>
<%--                                <li <c:if--%>
<%--                                        test="${variantStyle.url eq currentStyleUrl}"> class="active"</c:if>>--%>
<%--                                    <c:if test="${variantStyle.url eq currentStyleUrl}">--%>
<%--                                        <div id="currentStyleValue"--%>
<%--                                             data-style-value="${styleValueHtml}"></div>--%>
<%--                                    </c:if>--%>
<%--                                    <c:url value="${variantStyle.url}" var="productStyleUrl"/>--%>
<%--                                    <a href="${fn:escapeXml(productStyleUrl)}" class="colorVariant"--%>
<%--                                       name="${fn:escapeXml(variantStyle.url)}">--%>
<%--                                        <c:if test="${not empty imageData}">--%>
<%--                                            <img src="${fn:escapeXml(imageData.url)}"--%>
<%--                                                 title="${styleValueHtml}" alt="${styleValueHtml}"/>--%>
<%--                                        </c:if>--%>
<%--                                        <c:if test="${empty imageData}">--%>
<%--                                            <span class="swatch_colour_a"--%>
<%--                                                  title="${styleValueHtml}"></span>--%>
<%--                                        </c:if>--%>
<%--                                    </a>--%>
<%--                                </li>--%>
<%--                            </c:forEach>--%>
...
<%--                                            <c:set var="variantOptionQualifierValueHtml"--%>
<%--                                                   value="${fn:escapeXml(variantOptionQualifier.value)}"/>--%>
...
<%--                                        <c:set var="stockLevelHtml"><spring:theme--%>
...
<%--                                <c:set var="stockLevelHtml">${fn:escapeXml(variantOption.stock.stockLevel)}--%>
<%--                                    <spring:theme code="product.variants.in.stock"/></c:set>--%>
<%--                            </c:if>--%>
