	...
	<ycommerce:testId code="product_wholeProduct">
		...
		<div class="card-body">
			...
			<ycommerce:testId code="product_productName">
				<h5 class="card-title">${ycommerce:sanitizeHTML(product.name)}</h5>
				<p class="card-text">${ycommerce:sanitizeHTML(product.code)}</p>
			</ycommerce:testId>
		...
		<c:set var="isGrid" value="true" scope="request"/>
			...
			<div class="actions-container-for-${fn:escapeXml(component.uid)} <c:if test="${ycommerce:checkIfPickupEnabledForStore() and product.availableForPickup}"> </c:if>">
				</i><action:actions element="div" parentComponent="${component}"/>
			</div>
