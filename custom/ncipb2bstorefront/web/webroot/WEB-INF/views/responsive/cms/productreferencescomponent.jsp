...
<c:choose>
    <c:when test="${not empty productReferences and component.maximumNumberProducts > 0}">
        ...
        <h3 class="my-4">${fn:escapeXml(component.title)}</h3>

        <div class="row js-owl-carousel js-owl-lazy-reference js-owl-carousel-reference">
            ...
            <c:forEach end="${component.maximumNumberProducts}" items="${productReferences}"
                       var="productReference">
                <c:url value="${productReference.target.url}/quickView" var="productUrl"/>
                <div class="item">
                    <a href="${fn:escapeXml(productUrl)}" class="js-reference-item"
                       data-quickview-title="<spring:theme code="popup.quick.view.select"/></span>">
						<div class="thumb">
                        <product:productPrimaryReferenceImage product="${productReference.target}"
                                                              format="product"/>
						</div>
